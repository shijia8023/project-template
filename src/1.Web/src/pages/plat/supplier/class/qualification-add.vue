<template>
    <sapi-dialog v-model="visible" width="500px" top="26%" class="add" @on-open="open" @on-close="close">
        <span slot="title" v-text="$t('supplierClass.addSupplierQualificationTitle')"></span>
...
<script>
    ...
    export default {
        ...
        props: ["value","option","rowIndex"],
        methods: {
            open() {
                this.qualificationModel.QualificationName = "";
                this.qualificationModel.RowIndex = this.rowIndex;
                this.qualificationModel.Remark = "";
            },
            close() {
            ...
            confirm() {
				if(this.validate()) {
					this.$emit('callback', JSON.parse(JSON.stringify(this.qualificationModel)));
					this.close();
				}
			},
            validate() {
                ...
                if(this.checkQualificationNamesExists(this.qualificationModel.QualificationName)) {
            ...
            //校验资质名称是否已存在
			checkQualificationNamesExists(val) {
				var isExists = false;
				var supplierQualifications = this.option;
				if(supplierQualifications && supplierQualifications.length > 0) {
                    for(var i = 0, ii = supplierQualifications.length; i < ii; i++) {
						...
						if(supplierQualification&&supplierQualification.QualificationName == val) {
							isExists = true;
							break;
						}
					}
