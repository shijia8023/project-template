<template>
    <sapi-dialog v-model="visible" width="1200px" top="10%" class="add" @on-open="open" @on-close="close">
        <span slot="title" v-text="$t('supplierManage.addSupplierTitle')"></span>
			...
			<div class="form-collapses">
				<div class="fullline">
					<div class="wp-50">
						...
						<div class="right-auto-box">
							<el-input id="txtSupName" v-model.trim="createModel.SupName" :maxlength="100"></el-input>
						</div>
					</div>
				...
				<div class="fullline">
					...
					<div class="wp-50 float-right">
						...
						<div class="right-auto-box">
							<el-input id="txtCreditCode" v-model.trim="createModel.CreditCode" :maxlength="18"></el-input>
						</div>
					</div>    
				</div>
				...
				<div class="fullline">
					...
					<div class="right-auto-box">
						<sapi-select-btn v-model="createModel.SupplierClassRelats" :props="classRelatProps" height="56px" :has-more="true" @select="selectClassRelat"></sapi-select-btn>
					</div>
				</div>
				...
				<div class="fullline">
					...
					<div class="right-auto-box">
							...
							:has-size="false" v-model="businessLicenceUrlModel"></sapi-upload>
					</div>
				</div>
				...
				<div class="fullline">
					<div class="wp-50">
						...
						<div class="right-auto-box"> 
							<sapi-select v-model="createModel.EnterpriseForm" :data="enterpriseFormValues"
					...
					<div class="wp-50 float-right">
						...
						<div class="right-auto-box">
							<sapi-select v-model="createModel.TaxPayerId" :data="taxPayerValues"
				...
				<div class="fullline">
					<div class="wp-50">
						...
						<div class="right-auto-box">
							<el-input id="txtLegalRepresentative" v-model.trim="createModel.LegalRepresentative" :maxlength="20"></el-input>
						</div>
					</div>
					<div class="wp-50 float-right">
						<span class="w-65 " v-text="$t('supplierManage.registeredCapital')"></span>
						<div class="right-auto-box">
							<el-input id="txtRegisteredCapital" v-model.trim="createModel.RegisteredCapital" :maxlength="50"></el-input>
						</div>
					</div>  
				</div>
				...
				<div class="fullline">
					<div class="wp-50">
						...
						<div class="right-auto-box">
							<el-input id="txtAddress" v-model.trim="createModel.Address" :maxlength="1000"></el-input>
						</div>
					</div>
					<div class="wp-50 float-right">
						<span class="w-65 " v-text="$t('supplierManage.postalCode')"></span>
						<div class="right-auto-box">
							<el-input id="txtPostalCode" v-model.trim="createModel.PostalCode" :maxlength="20"></el-input>
						</div>
					</div>  
				</div>
				...
				<div class="fullline">
					...
					<div class="wp-50 float-right">
						...
						<div class="right-auto-box">
							<el-input id="txtFaxNumber" v-model.trim="createModel.FaxNumber" :maxlength="100"></el-input>
						</div>
					</div>  
				</div>
				...
				<div class="fullline">
					...
					<div class="wp-50 float-right">
						...
						<div class="right-auto-box">
							<el-input id="txtOfficialWebsite" v-model.trim="createModel.Supplier" :maxlength="1000"></el-input>
						</div>
					</div>  
				</div>
    ...
    export default {
		...
		computed:{
			tableQualficationDatas(I) {
				let arrs=[];
				if(this.createModel.SupplierClassRelats) {
					this.createModel.SupplierClassRelats.forEach(row=>{
        ...
        methods: {
			...
			selectClassRelat() {
				this.contactOption=this.createModel.SupplierClassRelats;
				this.contactVisible=true;
					...
					if( res[1]=="contactAdd") {
						...
						if(this.tableQualficationData.length>0) {
							...
							for(var i=0;i<res[0].length;i++)
							{
								...
								{
									if(this.tableQualficationData[j].QualificationId == res[0][i].QualificationId) {
										isAdd = false;
									}
								}
								if(isAdd) {
						...
						this.createModel.SupplierClassRelats=res[0];
					}
				}
			},
			...
			rowQualificationSelect(datas) {
				...
				if(datas.length > 0) {
					datas.forEach(function(data) {
						_this.qualificationDeleteIds.push(data.QualificationId);
					});
				}
			},
			...
			deleteQualificationItems() {
				...
				for(var i=_this.tableQualficationData.length-1;i>-1;i--) {
					var qualificationId=_this.tableQualficationData[i].QualificationId;
					var index=ids.indexOf(qualificationId);
					if(index>-1) {
			...
			openAddQualificationDialog() {
				if(this.createModel.SupplierClassRelats.length==0) {
					Vue.msg(this.$t("supplierManage.classNameNotBeEmpty"));
					return;
				}
				let classIds = "";
				this.createModel.SupplierClassRelats.forEach(function(data) {
            ...
            submit() {
                ...
                if(this.businessLicenceUrlModel.length >0) {
                    this.createModel.BusinessLicence = this.businessLicenceUrlModel[0].FilePath;
                }
				//联系人信息
				this.createModel.SupplierContacts=this.tableContactData;
				//重新构建资质文件
				this.qualificationFileModel=[];
				for(var i=0;i<this.tableQualficationData.length;i++)
				{
					var _this=this.tableQualficationData[i];
					let qualificationId=_this.QualificationId;
					if(_this[qualificationId].length>0) {
						let qualificationName=_this.QualificationName;
						let classId=_this.ClassId;
						for(var j=0;j<_this[qualificationId].length;j++)
						{
							...
							supplierQualificationFile.FileName=_this[qualificationId][j].FileName;
							supplierQualificationFile.OriginalFilePath=_this[qualificationId][j].FilePath;
				...
				this.createModel.SupplierQualificationFiles=this.qualificationFileModel;
				//合作案例
				this.createModel.SupplierCooperativeCases=this.tableCaseData;
                ...
                this.$post("/api/plat/suppliers/", this.createModel, function (res) {
					...
					this.disabled=false;
					this.close();
					Vue.successMsg(this.$t('supplierManage.addSupplierSuccess'));
                });
            },
            validate() {
                this.$closeWaringTips(".form-error-tips");
				...
				if(this.createModel.CreditCode && !this.$checkedASCII(this.createModel.CreditCode)) {
				...
				if(!this.createModel.BusinessLicence) {
					Vue.msg(this.$t("supplierManage.businessLicenceNotBeEmpty"));
					return false;
				}
				if(this.createModel.PostalCode && !this.$checkedASCII(this.createModel.PostalCode)) {
				...
				if(this.createModel.FaxNumber && !this.$checkedASCII(this.createModel.FaxNumber)) {
